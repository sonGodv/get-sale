...
Процедура ЗагрузитьНастройки()
	...
	Если ФайлНастроек.ФайлСуществует() Тогда
		...
		Если СокрЛП(Файл.ПолучитьСтроку(8)) <> "0" тогда
			СпрК = Справочники.Клиенты.НайтиПоКоду(СокрЛП(Файл.ПолучитьСтроку(8)));
		...
		Если СокрЛП(Файл.ПолучитьСтроку(9)) <> "0" тогда
			СпрС = Справочники.Склады.НайтиПоКоду(СокрЛП(Файл.ПолучитьСтроку(9)));
		...
		КонецЕсли;           
			...
			СпрФ = Справочники.Фирмы.НайтиПоКоду(СокрЛП(Файл.ПолучитьСтроку(10)));
...
Процедура ЗаказыПолучить()
	...
	ТабЗаказов.Колонки.Добавить("order_id",,"order_id",);
	ТабЗаказов.Колонки.Добавить("customer_id",,"customer_id",);
	ТабЗаказов.Колонки.Добавить("firstname",,"Имя",);
	ТабЗаказов.Колонки.Добавить("lastname",,"Фамилия",);
	ТабЗаказов.Колонки.Добавить("email",,"email",);
	ТабЗаказов.Колонки.Добавить("telephone",,"telephone",);
	ТабЗаказов.Колонки.Добавить("payment_method",,"Метод оплаты",);
	ТабЗаказов.Колонки.Добавить("payment_code",,"Код оплаты",);
	ТабЗаказов.Колонки.Добавить("shipping_method",,"Метод отправки",);
	ТабЗаказов.Колонки.Добавить("shipping_firstname",,"Имя получателя",);	
	ТабЗаказов.Колонки.Добавить("shipping_lastname",,"Фамилия получателя",);
	ТабЗаказов.Колонки.Добавить("shipping_address_1",,"Адрес доставки",);
	ТабЗаказов.Колонки.Добавить("shipping_city",,"Город",);
	ТабЗаказов.Колонки.Добавить("shipping_postcode",,"Индекс/Отделение",);	
	ТабЗаказов.Колонки.Добавить("shipping_zone",,"Область",);	
	ТабЗаказов.Колонки.Добавить("shipping_code",,"Метод доставки",);	
	ТабЗаказов.Колонки.Добавить("comment",,"Комментарий",);
	ТабЗаказов.Колонки.Добавить("total",,"Сумма",);
	ТабЗаказов.Колонки.Добавить("order_status_id",,"Статус заказа",);
	ТабЗаказов.Колонки.Добавить("ip",,"ip Пользователя",);	
	ТабЗаказов.Колонки.Добавить("po_date_added",,"Дата заказа",);	
	...
	Пока Заказ.Eof() = 0 Цикл
		...
		НС_ТабЗаказов = ТабЗаказов.Добавить();
	...
	Для ы=1 по табЗаказ.КоличествоСтрок() Цикл
		Если спрЗаказы.НайтиПоРеквизиту("id",табЗаказ.ПолучитьЗначение(ы,"order_id"),1) = 0 тогда
			...
			иначе
				...
				спрКл.ВидТорговли = Перечисления.ВидыТорговли.Предоплата;
				спрКл.ВозСкидка = Перечисления.ВариантыСкидок.БезСкидки;
				спрКл.ЗапрВыдачи = Перечисления.ДаНет.Нет; 
				спрКл.ТипЦен = Перечисления.ТипыЦен.Категория2;
...
Процедура СформироватьНакладную(ДанныеДляДокумента)
	...
	Если Клиент.ТипКонтрагента = Перечисления.ТипКонтрагента.ТТ тогда
	...
	док.ВидУчета = Перечисления.ВидыУчета.Совместный;
	...
	Для ы = 1 По ДанныеДляДокумента.КоличествоСтрок() Цикл
		...
		Если НомСтрокДок > 2 тогда
			...
			Если спрТов.НайтиПоКоду(sku,0) = 1 тогда
				...
				иначе
					Если док.Клиент.ТипЦен = Перечисления.ТипыЦен.Категория1 тогда 
						...
						КонецЕсли;
					КонецЕсли;
					Если док.Клиент.ТипЦен = Перечисления.ТипыЦен.Категория2 тогда 
						...
						КонецЕсли;
					КонецЕсли;
					Если док.Клиент.ТипЦен = Перечисления.ТипыЦен.Категория3 тогда
				...
				Если (док.ТМЦ.Вид = Перечисления.ВидыТМЦ.Товар) или (док.ТМЦ.Вид = Перечисления.ВидыТМЦ.Телефон)  тогда
					...
					Пока спрПартии.ПолучитьЭлемент() = 1 Цикл
						Если (РегистрыНакопления.Остатки.СводныйОстаток(0,док.ТМЦ,док.Склад,спрПартии.ТекущийЭлемент(),"Кво")>док.Кво) или (РегистрыНакопления.Остатки.СводныйОстаток(0,док.ТМЦ,док.Склад,спрПартии.ТекущийЭлемент(),"Кво")=док.Кво) тогда
...
Процедура ВыгрузитьОстатки()
	...
	Пока ЗапросНоменклатура.Группировка(1) = 1 Цикл
		...
		Если Номенклатура.ЭтоГруппа() = 0 тогда
			...
			Остаток = РегистрыНакопления.Остатки.СводныйОстаток(0,НоменклатураТМЦ,Константы.БазСклад,,"Кво");
...
Процедура ПриОткрытии()
	...
	Если ПустоеЗначение(ЭтаФорма.Параметр) = 0 тогда
		Если ЭтаФорма.Параметр = "1" тогда
			ВыгрузитьОстатки();
		иначе
			ДанныеДляДокумента = ЭтаФорма.Параметр;
